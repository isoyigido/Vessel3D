...
public class Converter {
    ...
    public static int[][][] convertToInt3(BufferedImage[] slices, int[][][] outer_layer) {
        int[][][] result = new int[slices.length][slices[0].getHeight()][slices[0].getWidth()];

        outer_layer = removeInnerUnits(outer_layer);
        ...
        for (int z = 0; z < slices.length; z++) {
            ...
            for (int y = 0; y < slice.getHeight(); y++) {
                for (int x = 0; x < slice.getWidth(); x++) {
                    if (slice.getRGB(x, y) < lowerBrightnessLimit && outer_layer[z][y][x] == 0) {
                        result[z][y][x] = 0;
                    } else {
                        result[z][y][x] = 1;
                    }
